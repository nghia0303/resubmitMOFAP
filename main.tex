...
The MO-FAP is explicitly formulated by the objective function~\eqref{eq:ilp_obj} and constraints~\eqref{eq:ilp_domain}--\eqref{eq:ilp_ybin}.
\begin{align} 
...
\begin{align}
    & b_{v,j} \leftrightarrow g_{v,j} \wedge \neg g_{v,j+1}, \quad \forall v \in V, \forall j \in \{1, \dots, |D_v|-1\} \label{eq:g_b_connect} \\
...
% TODO: Revise the L/R boundary presentation.
% 1) Add figure/text cases for D_w entirely left or right of the forbidden interval; these cases generate no POSE constraint.
% 2) State that an empty D_w after preprocessing means infeasibility and no SAT encoding is constructed.
% 3) State that if all frequencies in D_w are inside the forbidden interval, D_{v,j} is removed from D_v by preprocessing.
% 4) Define L and R only when the corresponding safe side exists; avoid argmax/argmin over an empty set.
% 5) Split eqs. forbid_1--forbid_3 into three separate cases and state which boundary index exists in each case.
...
Subsequently, the order variables $g$ are used to reformulate the interference constraints. When transmitter $v$ is assigned the frequency $D_{v,j}$ (i.e., $b_{v,j}=\text{True}$), it induces the forbidden interval $[D_{v,j}-d_{vw},\,D_{v,j}+d_{vw}]$ for its adjacent transmitter $w$. At this stage, the preprocessed domain $D_w$ is nonempty. Relative to this forbidden interval, the ordered domain $D_w$ may lie entirely on the left, overlap on the left, extend on both sides, overlap on the right, or lie entirely on the right. If $D_w$ lies entirely on the left or entirely on the right, then every value in $D_w$ already satisfies the interference constraint, and no additional POSE implication is required. If all values in $D_w$ fall inside the forbidden interval, then $D_{v,j}$ has no compatible value in $D_w$ and is therefore removed from $D_v$ during preprocessing. The remaining overlapping cases are encoded by boundary indices on the safe side(s) of $D_w$ outside the forbidden interval.
...
\node[font=\bfseries, text=blue] at (4.5, 11.7) {$\Rightarrow \text{No POSE implication}$};

% ==========================================
% Case 2: D_w overlaps the forbidden interval on the left
\drawBase{10}{Case 2}
\draw[domain_valid] (0.5, 10.3) -- (\fMin, 10.3);
\draw[domain_invalid] (\fMin, 10.3) -- (5.5, 10.3);
\node[domain_bound, green!60!black, label=above:{$D_{w,1}$}] at (0.5, 10.3) {};
\node[green!60!black] at (1.25, 10.3) {$\dots$};
\node[domain_bound, green!60!black, label=above:{$L$}] at (2.0, 10.3) {};
\node[domain_bound, red, label=above:{$L+1$}] at (3.0, 10.3) {};
\node[red] at (4.25, 10.3) {$\dots$};
\node[domain_bound, red, label=above:{$D_{w,|D_w|}$}] at (5.5, 10.3) {};
\node[font=\bfseries, text=blue] at (4.5, 9.2) {$\Rightarrow b_{v,j} \rightarrow \neg g_{w,L+1}$};

% ==========================================
% Case 3: D_w extends on both sides of the forbidden interval
\drawBase{7.5}{Case 3}
\draw[domain_valid] (0.5, 7.8) -- (\fMin, 7.8);
\draw[domain_invalid] (\fMin, 7.8) -- (\fMax, 7.8);
\draw[domain_valid] (\fMax, 7.8) -- (8.5, 7.8);
...
% ==========================================
% Case 5: D_w lies entirely to the right of the forbidden interval
\drawBase{2.5}{Case 5}
\draw[domain_valid] (7.0, 2.8) -- (8.5, 2.8);
\node[domain_bound, green!60!black, label=above:{$D_{w,1}$}] at (7.0, 2.8) {};
\node[green!60!black] at (7.75, 2.8) {$\dots$};
\node[domain_bound, green!60!black, label=above:{$D_{w,|D_w|}$}] at (8.5, 2.8) {};
\node[font=\bfseries, text=blue] at (4.5, 1.7) {$\Rightarrow \text{No POSE implication}$};
...
\caption{Visualization of the six relative positions between the forbidden interval $[D_{v,j}-d_{vw},D_{v,j}+d_{vw}]$ and the domain $D_w$. Red segments denote forbidden frequencies of $w$, whereas green segments denote admissible frequencies. Cases 1 and 5 require no additional POSE implication because every value in $D_w$ already satisfies the interference constraint. Cases 2--4 are encoded by the boundary implications in~\eqref{eq:forbid_2},~\eqref{eq:forbid_3}, and~\eqref{eq:forbid_1}, respectively. In Case 6, all frequencies in $D_w$ are forbidden, so $D_{v,j}$ is removed from $D_v$ by preprocessing.}
\label{fig:domain_pruning_cases}
\end{figure}

% TODO-POSE-rewrite-outline:
% 1) Case explanation paragraph:
%    - Use Fig.~\ref{fig:domain_pruning_cases} to explain where the safe regions of D_w lie relative to the forbidden interval induced by D_{v,j}.
%    - Case 1: only the right safe region remains; g_{w,R} forces w to choose an index at least R.
%    - Case 2: only the left safe region remains; \neg g_{w,L+1} forces w to choose an index at most L.
%    - Case 3: the forbidden interval lies inside the range of D_w, so safe regions exist on both sides; \neg g_{w,L+1} \lor g_{w,R} allows either the left or the right side.
%    - Add Case 4 to the figure: all candidate frequencies in D_w lie inside the forbidden interval. Label it as "eliminated by preprocessing"; it has no POSE clause because D_{v,j} would have been removed from D_v by \eqref{eq:pre_ineq}.
%    - Avoid informal/overclaiming words such as jumping, retreating, drastically, seamlessly, and cumbersome.
%
% 2) Correctness transition paragraph:
%    - State that DSE explicitly forbids each pair with |D_{v,j}-D_{w,j'}| <= d_{vw}.
%    - State that POSE encodes the same admissible choices for w using the boundary indices L and R instead of enumerating all forbidden pairs.
%    - Explicitly handle Case 4 in the proof: if the whole D_w is forbidden for D_{v,j}, then D_{v,j} has no compatible value in D_w and preprocessing removes it by \eqref{eq:pre_ineq}.
%    - Lead into Proposition~\ref{prop:pose_correctness}, which should prove equivalence between POSE boundary implications and DSE interference clauses.
%
% 3) Complexity paragraph, preferably after Proposition~\ref{prop:pose_correctness}:
%    - DSE may generate O(|D_v||D_w|) conflict clauses for an interference edge vw.
%    - POSE generates O(1) boundary clauses per candidate assignment b_{v,j}, not O(1) per edge.
%    - Therefore directed constraints from v to w require O(|D_v|) boundary clauses.
%    - If both directions are generated, an undirected edge vw requires O(|D_v|+|D_w|) boundary clauses.
%    - Order variables and consistency constraints are linear in the total domain size.
