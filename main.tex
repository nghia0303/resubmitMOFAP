...
& \quad Set $k\leftarrow UB-1$ and solve $F$ with the corresponding assumption: the \texttt{ITotalizer} assumption $-\texttt{rhs}[k]$ enforcing At-Most-$k$ for INC, or $\neg S_{N,k+1}$ for INCSC. \\
...
The SAT-based methods differ in terms of how the feasibility constraints and the objective function are handled. For the feasibility constraints, DSE and POSE correspond to the encodings described in Sections~\ref{subsec:sat_encoding} and~\ref{subsec:order_encoding}, respectively, while CARD uses PySAT cardinality encodings for~\eqref{eq:sat_domain} and~\eqref{eq:sat_ineq}. For the objective function, INC and INCSC correspond to the incremental SAT strategies described in Section~\ref{subsec:incremental_sat}.
The proposed framework is further compared against Gurobi, CPLEX-CP, CPLEX-MIP, CP-SAT, and MaxSAT+POSE. Table~\ref{tab:method_notation} summarizes the method names used in the experimental results.

Each run is reported with one of three final statuses. \texttt{OPT} denotes that the solver certifies the minimum number of assigned frequencies, \texttt{INF} denotes that the instance is proven infeasible, and \texttt{TO} denotes that the run reaches the 600-second time limit without a certificate. When total solving time is computed, each \texttt{TO} run is counted as 600 seconds. Details on the benchmark data, experimental results, and source code are provided in the Data and Code Availability statement.
...
CARD & Cardinality-based SAT encoding for~\eqref{eq:sat_domain} and~\eqref{eq:sat_ineq} using PySAT Exactly-One and At-Most-One encodings. \\
...
\midrule
\multicolumn{2}{@{}l}{\textit{SAT configurations}} \\
DSE+INCSC & DSE combined with INCSC. \\
POSE+INC & POSE combined with INC. \\
POSE+INCSC & POSE combined with INCSC. (Main proposed SAT configuration)\\
\midrule
...
MaxSAT+POSE & MaxSAT formulation with POSE constraints solved by RC2. \\
\bottomrule
\end{tabular}
\end{table}
...
This experiment evaluates the contribution of domain preprocessing to the solving process. As shown in Fig.~\ref{fig:preprocess_scatter}, POSE+INCSC is faster with preprocessing on 28 of the 35 instances. The effect is particularly visible on the 13 infeasible (\texttt{INF}) instances, where preprocessing reduces the time required to prove infeasibility for both POSE+INCSC and DSE+INCSC.
The preprocessing step also affects the other exact solvers considered in this study. For CPLEX-MIP and CPLEX-CP, it avoids large slowdowns on several difficult instances. For Gurobi and CPLEX-MIP, it reduces the solving time on all infeasible instances. Overall, most points in Fig.~\ref{fig:preprocess_scatter} lie below the $y=x$ line, indicating that preprocessing is beneficial in this evaluation.
