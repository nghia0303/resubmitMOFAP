...
Prior to the SAT encoding, an arc-consistency preprocessing step \cite{Aardal2007, Dib2009} is applied to prune invalid frequencies from the initial domains $D_v$. Note that pre-assigned transmitters are natively handled since their domains are already initialized as singletons ($D_v = \{p_v\}$). For the remaining transmitters, their domains are iteratively filtered based on the network's interference constraints.

Specifically, a frequency $f \in D_v$ is retained only if it is supported by at least one valid frequency $h \in D_w$ at the adjacent vertex. For interference and bidirectional constraints, the domain update rules are strictly tightened as formulated in~\eqref{eq:pre_ineq} and~\eqref{eq:pre_eq}, respectively.

\begin{align}
    D_v &\leftarrow \{ f \in D_v \mid \exists h \in D_w : |f - h| > d_{vw} \}, \label{eq:pre_ineq} \\
    D_v &\leftarrow \{ f \in D_v \mid \exists h \in D_w : |f - h| = d_{vw} \}. \label{eq:pre_eq}
\end{align}
These reduction rules are applied bidirectionally across all edges and iteratively repeated until a fixed point is reached (i.e., no further domain reductions are possible). By eliminating infeasible frequencies early, this standard arc-consistency procedure drastically shrinks the search space, which subsequently minimizes the number of Boolean variables and clauses generated in the SAT model, accelerating the overall solving process.
